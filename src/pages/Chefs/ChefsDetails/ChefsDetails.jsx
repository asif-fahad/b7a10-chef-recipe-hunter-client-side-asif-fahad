import React from 'react';
import { useLoaderData } from 'react-router-dom';
import { Rating } from '@smastrom/react-rating'
import '@smastrom/react-rating/style.css'

const ChefsDetails = () => {
    const details = useLoaderData();

    const { id, chef_photo, chef_name, experience, recipes_number, recipe_name, likes, description, recipe_one, recipe_two, recipe_three, ingredients, method, rating } = details;
    return (
        <div>
            <div>
                <h1 className='text-4xl text-center m-0 bg-gray-200 p-10'>Chef Details</h1>
            </div>
            <div className='mt-5'>
                <div className="card bg-base-100 shadow-xl">
                    <figure><img src={chef_photo} alt="chefs" className=' object-contain' /></figure>
                    <div className="card-body">
                        <h2 className="card-title">{chef_name}</h2>
                        <p><span className='font-semibold' >Description:</span> {description}</p>
                        <p><span className='font-semibold' >Experience:</span> {experience}</p>
                        <p><span className='font-semibold' >Recipes:</span> {recipes_number}</p>


                        {/* <p><span className='font-semibold' >Recipe Name:</span> {recipe_name}</p> */}
                        <div className='flex gap-1'>
                            <div><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M6.633 10.5c.806 0 1.533-.446 2.031-1.08a9.041 9.041 0 012.861-2.4c.723-.384 1.35-.956 1.653-1.715a4.498 4.498 0 00.322-1.672V3a.75.75 0 01.75-.75A2.25 2.25 0 0116.5 4.5c0 1.152-.26 2.243-.723 3.218-.266.558.107 1.282.725 1.282h3.126c1.026 0 1.945.694 2.054 1.715.045.422.068.85.068 1.285a11.95 11.95 0 01-2.649 7.521c-.388.482-.987.729-1.605.729H13.48c-.483 0-.964-.078-1.423-.23l-3.114-1.04a4.501 4.501 0 00-1.423-.23H5.904M14.25 9h2.25M5.904 18.75c.083.205.173.405.27.602.197.4-.078.898-.523.898h-.908c-.889 0-1.713-.518-1.972-1.368a12 12 0 01-.521-3.507c0-1.553.295-3.036.831-4.398C3.387 10.203 4.167 9.75 5 9.75h1.053c.472 0 .745.556.5.96a8.958 8.958 0 00-1.302 4.665c0 1.194.232 2.333.654 3.375z" />
                            </svg> </div>
                            <div>{likes}</div>
                        </div>
                        <div className='grid grid-cols-1 lg:grid-cols-3'>
                            <div className="card card-compact w-96 bg-base-100 shadow-xl">
                                <div className="card-body">
                                    <div>
                                        <p><span className='font-semibold' >Recipe One:</span> {recipe_one}</p>
                                        <p><span className='font-semibold' >Ingredients:</span> {ingredients.map(i => <li>{i}</li>)}</p>
                                        <p><span className='font-semibold' >Method: </span> {method}</p>
                                        <div className='flex'>
                                            <Rating
                                                style={{ maxWidth: 100 }}
                                                value={Math.round(rating?.number || 0)} readOnly />
                                            <span className='ms-2'> {rating?.number}</span>
                                        </div>
                                        <button className='btn w-52'>Favorite</button>
                                    </div>
                                </div>
                            </div>
                            <div className="card card-compact w-96 bg-base-100 shadow-xl">
                                <div className="card-body">
                                    <div>
                                        <p><span className='font-semibold' >Recipe Two:</span> {recipe_two}</p>
                                        <p><span className='font-semibold' >Ingredients:</span> {ingredients.map(i => <li>{i}</li>)}</p>
                                        <p><span className='font-semibold' >Method: </span> {method}</p>
                                        <div className='flex'>
                                            <Rating
                                                style={{ maxWidth: 100 }}
                                                value={Math.round(rating?.number || 0)} readOnly />
                                            <span className='ms-2'> {rating?.number}</span>
                                        </div>
                                        <button className='btn w-52'>Favorite</button>
                                    </div>
                                </div>
                            </div>
                            <div className="card card-compact w-96 bg-base-100 shadow-xl">
                                <div className="card-body">
                                    <div>
                                        <p><span className='font-semibold' >Recipe Three:</span> {recipe_three}</p>
                                        <p><span className='font-semibold' >Ingredients:</span> {ingredients.map(i => <li>{i}</li>)}</p>
                                        <p><span className='font-semibold' >Method: </span> {method}</p>
                                        <div className='flex'>
                                            <Rating
                                                style={{ maxWidth: 100 }}
                                                value={Math.round(rating?.number || 0)} readOnly />
                                            <span className='ms-2'> {rating?.number}</span>
                                        </div>
                                        <button className='btn w-52'>Favorite</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default ChefsDetails;